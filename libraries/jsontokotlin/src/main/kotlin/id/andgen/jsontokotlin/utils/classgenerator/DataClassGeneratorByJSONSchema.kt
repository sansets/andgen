...
import id.andgen.jsontokotlin.JSON_SCHEMA_FORMAT_MAPPINGS
import id.andgen.jsontokotlin.model.classscodestruct.*
import id.andgen.jsontokotlin.utils.constToLiteral
import id.andgen.jsontokotlin.model.codeelements.getDefaultValue
import id.andgen.jsontokotlin.model.jsonschema.JsonObjectDef
import id.andgen.jsontokotlin.model.jsonschema.JsonSchema
import id.andgen.jsontokotlin.model.jsonschema.PropertyDef
import java.util.*
...
class DataClassGeneratorByJSONSchema(private val rootClassName: String, private val jsonSchema: JsonSchema) {
    ...
    fun generate(): DataClass {
        val className = rootClassName.ifBlank { jsonSchema.title ?: "" }
    ...
    private fun resolveEnumClass(enumDef: PropertyDef, name: String): KotlinClass {
        ...
        if (enumDef.xEnumNames != null && enumDef.enum.count() != enumDef.xEnumNames.count())
            throw IllegalArgumentException("$name enum values count ${enumDef.enum.count()} not equal to enum names count ${enumDef.xEnumNames}")
        ...
        return EnumClass(
            ...
            xEnumNames = enumDef.xEnumNames?.asList(),
            generic = typeClass,
    ...
    private fun resolveSealedClass(def: PropertyDef, name: String): KotlinClass {
        ...
        val referencedDefinitions = realDef.anyOf?.associate {
